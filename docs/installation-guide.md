...
```sh
git clone https://github.com/adeilhamfajri/webRTC.git ./
npm install
```
...
Open this URL: `https://localhost:80/socket.io/socket.io.js`

If you can load `/socket.io/socket.io.js` on your server then it is working fine.

# Stop Old Processes

Check all processes running on port `80` and stop process by `id`:
